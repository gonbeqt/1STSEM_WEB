...
import {
  Audit,
  AuditContractResponse,
  AuditDetailsResponse,
  AuditRequestData,
  AuditStatisticsResponse,
  UploadContractResponse,
} from '../../domain/entities/ContractEntities';

const API_URL = process.env.REACT_APP_API_BASE_URL;

export class ContractRemoteDataSource {
  ...
  async uploadContract(file: File): Promise<UploadContractResponse> {
    ...
    try {
      const response = await this.api.postForm<UploadContractResponse>(`${API_URL}/ai/upload-contract/`, formData);
      return response;
    } catch (error: any) {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        return {
          ...
          vulnerabilities: [
            { title: 'Reentrancy Vulnerability', description: 'Mock vulnerability description', severity: 'HIGH' },
            { title: 'Integer Overflow', description: 'Mock vulnerability description', severity: 'MEDIUM' },
            { title: 'Unchecked External Call', description: 'Mock vulnerability description', severity: 'LOW' },
          ],
        };
      }
    ...
    try {
      ...
      if (error.response?.status === 404) {
        return {
          ...
          vulnerabilities: [
            { title: 'Mock Vulnerability 1', description: 'Mock description', severity: 'HIGH' },
          ],
        };
      }
