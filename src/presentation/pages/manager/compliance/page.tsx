...
const CompliancePage: React.FC = () => {
    ...
    return (
        <div className="max-w-2xl mx-auto my-8 p-8 bg-gray-50 rounded-lg shadow-sm">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button 
                    type="submit" 
                    className="w-full p-4 bg-blue-600 text-white border-none rounded text-lg font-semibold hover:bg-blue-700 transition-colors"
                >
                    Upload Documents
                </button>
            </form>
            {message && <p className={`text-center mt-4 font-bold ${message.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>{message}</p>}
        </div>
    );
};
